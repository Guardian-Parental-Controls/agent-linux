...
const SCRIPT_INTERPRETERS: &[&str] = &[
    "bash", "sh", "zsh", "fish", "python", "python3", "node", "ruby", "perl", "php",
];
...
pub async fn approval_policy_for_user(username: &str) -> Option<ApprovalPolicy> {
    ...
    guard.current_state.approval_overlays.get(username).cloned()
}
...
impl AppArmorRuntime {
    ...
    fn update_user_policy(
        ...
        println!(
            "apparmor: updating user policy for '{}' (received {} rules, approval_policy={})",
            username,
            policies.len(),
        ...
        if let Some(policy) = self
            ...
            .find(|policy| {
                policy.match_type == MATCH_TYPE_EXECUTABLE
                    ...
                    && normalized_executable_match_path(&policy.executable_path)
                        == normalized_policy_exe
            })
        {
            return Some(ExecDecision {
                preset: policy.preset.clone(),
        ...
        for entry in entries.flatten() {
            ...
            if file_name.starts_with(prefix) {
                ...
                match output {
                    ...
                    Err(e) => {
                        return Err(format!(
                            "failed to run apparmor_parser -R for {}: {}",
                            file_name, e
                        ));
                    }
                }
