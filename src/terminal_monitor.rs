...
"#;
    ...
    if let Err(e) = fs::write(sh_path, bash_zsh_hook) {
        eprintln!(
            "terminal_monitor: failed to write shell hook to {}: {}",
            sh_path, e
        );
    } else {
    ...
    if fs::create_dir_all(fish_dir).is_ok() {
        ...
        if let Err(e) = fs::write(&fish_path, fish_hook) {
            eprintln!(
                "terminal_monitor: failed to write fish hook to {}: {}",
                fish_path, e
            );
        } else {
    ...
    *offset = len;
    ...
    for (cmd, ts_opt) in entries {
        ...
        let occurred_at = if let Some(ts) = ts_opt {
            ...
            chrono::DateTime::<chrono::Utc>::from(UNIX_EPOCH + Duration::from_secs(now_ts))
                .to_rfc3339()
        };
