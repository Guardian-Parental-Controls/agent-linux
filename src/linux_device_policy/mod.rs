...
static LINUX_DEVICE_POLICY_RUNTIME: OnceLock<Arc<Mutex<LinuxDevicePolicyRuntime>>> =
    OnceLock::new();
...
pub async fn sync_user_policy(username: &str, payload: DevicePolicyPayload) -> Result<(), String> {
    let runtime = get_runtime();
    let mut guard = runtime.lock().await;
    guard.ensure_restored()?;
...
impl LinuxDevicePolicyRuntime {
    ...
    fn update_user_policy(
        ...
        println!(
            "linux_device_policy: updating device policy for user '{}'",
            normalized_username
        );
        self.current_state
            .users
    ...
    fn reconcile_enforcement(&mut self) -> Result<(), String> {
        println!(
            "linux_device_policy: reconciling rules. Active session username: {:?}",
            self.active_session_username
        );
        ...
        println!(
            ...
            active_user,
            payload.connectivity.bluetooth_disabled,
            payload.exec.terminal_access_disabled
        );
...
mod tests {
    ...
    fn active_managed_user_requires_catalog_entry() {
        ...
        assert_eq!(active_managed_user(Some("child"), &users), Some("child"));
