...
use guardian_agent::clock_integrity::{
    ClockIntegrityState, DetectionSource, PersistedClockIntegrity, TickStatus, apply_tick,
    fetch_tick_inputs,
};
#[cfg(target_os = "linux")]
use serde_json::json;
...
#[cfg(target_os = "linux")]
pub fn spawn_logind_resume_listener(
    ...
    tokio::spawn(async move {
        let connection = match zbus::Connection::system().await {
            ...
            Err(error) => {
                eprintln!(
                    "clock integrity resume listener: bus connect failed: {}",
                    error
                );
                return;
            }
        ...
        let mut sleep_stream = match proxy.receive_prepare_for_sleep().await {
            ...
            Err(error) => {
                eprintln!(
                    "clock integrity resume listener: subscribe failed: {}",
                    error
                );
                return;
            }
