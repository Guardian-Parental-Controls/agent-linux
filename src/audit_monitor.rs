...
async fn run_monitor_inner(
    ...
    while let Ok(Some(line)) = reader.next_line().await {
        ...
        if now.duration_since(last_prune) >= ALERT_DEDUP_WINDOW {
            last_prune = now;
            last_sent_by_key
                .retain(|_, timestamp| now.duration_since(*timestamp) < ALERT_DEDUP_RETENTION);
        }
...
mod tests {
    ...
    fn extract_quoted_field() {
        ...
        assert_eq!(extract_field(line, "operation="), Some("open".to_string()));
        assert_eq!(
            extract_field(line, "name="),
            Some("/etc/passwd".to_string())
        );
        assert_eq!(extract_field(line, "comm="), Some("steam".to_string()));
    }

    #[test]
    fn extract_unquoted_field() {
        let line = "apparmor=DENIED operation=exec profile=timekpr-bob-discord";
        assert_eq!(extract_field(line, "apparmor="), Some("DENIED".to_string()));
        assert_eq!(
            extract_field(line, "profile="),
