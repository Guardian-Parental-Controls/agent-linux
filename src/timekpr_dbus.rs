...
impl TimekprDbusClient {
    ...
    pub async fn set_time_left(
        ...
        let proxy = self.user_admin_proxy().await?;
        proxy
            .call("setTimeLeft", &(username, operation, seconds))
            .await
    ...
    pub async fn set_allowed_days(
        ...
        let proxy = self.user_admin_proxy().await?;
        proxy
            .call("setAllowedDays", &(username, allowed_days))
            .await
    ...
    pub async fn set_time_limit_for_days(
        ...
        let proxy = self.user_admin_proxy().await?;
        proxy
            .call("setTimeLimitForDays", &(username, day_limits))
            .await
                    ...
                    let inner_json = JsonValue::Object(
                        inner_map
                            ...
                            .map(|(inner_key, inner_value)| {
                                (inner_key, JsonValue::from(inner_value))
                            })
                            .collect(),
                    );
