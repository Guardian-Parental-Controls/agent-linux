...
#[cfg(target_os = "linux")]
const PACKAGED_APP_PREFIXES: &[&str] = &["/opt/", "/snap/", "/var/lib/flatpak/", "/var/lib/snapd/"];

#[cfg(target_os = "linux")]
const UTILITY_BASENAMES: &[&str] = &[
    "sleep", "lsusb", "lsblk", "lspci", "grep", "awk", "sed", "cat", "echo", "printf", "test", "[",
    "cut", "sort", "uniq", "head", "tail", "wc", "date", "env", "id", "uname", "which", "type",
    "true", "false", "readlink", "dirname", "basename", "seq", "tr", "xargs", "find", "stat",
    "file", "du", "df", "mount", "ps", "pgrep", "pkill", "kill", "killall", "nice", "nohup",
    "timeout", "watch", "tee", "mktemp", "touch", "mkdir", "rm", "mv", "cp", "ln", "chmod",
    "chown", "ls", "pwd", "cd", "sh", "bash", "zsh", "dash", "fish",
];
...
mod tests {
    ...
    fn filters_system_utilities_without_inventory() {
        ...
        assert!(!decide_reportable("alice", "/usr/bin/sleep", &index));
        assert!(!decide_reportable("alice", "/usr/bin/lsusb", &index));
    }
    ...
    fn reports_inventory_basename_match_for_system_path() {
        ...
        assert!(decide_reportable("alice", "/usr/bin/firefox", &index));
    }
    ...
    fn reports_executables_under_user_home() {
        ...
        let exe = format!("{}/.local/bin/mygame", user_record.home_dir().display());
        let index = ReportableIndex::default();
    ...
    fn filters_system_python_without_policy_or_inventory() {
        let index = ReportableIndex::default();
        assert!(!decide_reportable("alice", "/usr/bin/python3", &index));
    }

    #[test]
    fn reports_custom_opt_installations() {
        let index = ReportableIndex::default();
        assert!(decide_reportable("alice", "/opt/steam/steam", &index));
    }
